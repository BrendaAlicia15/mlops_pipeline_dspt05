...
def build_model(
    ...
    #    train_and_select_best_model() # O la función de entrenamiento que tengas
    #    print("¡Entrenamiento finalizado y modelo guardado exitosamente!")

if __name__ == "__main__":
        from xgboost import XGBClassifier
        from cargar_datos import cargarDatos
        from ft_engineering import TARGET, COLUMNAS_NUMERIC, COLUMNAS_CATEGORIC, COLUMNAS_ORDINAL

        print("--- Iniciando proceso de entrenamiento y diagnóstico ---")
    
        # 1. Cargar dataset
        df = cargarDatos()
    
        # 2. Configurar diccionario de datos
        data_params = {
            "name_of_y_col": TARGET,
            "names_of_x_cols": COLUMNAS_NUMERIC + COLUMNAS_CATEGORIC + COLUMNAS_ORDINAL,
            "dataset": df
        }

        # 3. Calcular la proporción para el desbalance de clases (0: no pago, 1: pago)
        # Revisa si tu clase de interés 'no pago' es 0 o 1
        num_negativos = (df[TARGET] == 0).sum()
        num_positivos = (df[TARGET] == 1).sum()
    
        # ratio = cantidad_mayoria / cantidad_minoria
        ratio = num_positivos / num_negativos

        # 4. Instanciar el clasificador con el peso ajustado
        classifier = XGBClassifier(
            n_estimators=100,
            learning_rate=0.05,
            max_depth=3,      # Reducir profundidad para evitar overfitting
            scale_pos_weight=ratio,    # Ajusta el peso de la clase minoritaria
            random_state=1234,
            eval_metric="logloss"
        )

        # 5. Ejecutar el entrenamiento y desplegar gráficas
        ...
        print("Métricas en Entrenamiento:", resultados["train"])
        print("Métricas en Prueba:", resultados["test"])
        print("\nProceso finalizado y modelo guardado en modelo_pipeline.pkl.")